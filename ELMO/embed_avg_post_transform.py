...
if (len(sys.argv) < 7 or len(sys.argv) > 7):
    print(len(sys.argv))
    print("Usage: python dump_token_embeddings.py input_file vocab_file token_embedding_file output_file lang")
    sys.exit(1)
    ...
    for i in range(0,len(tokenized_txt), batch_size): 
     ...
     #perform averaging here ... 
      ...
      for x, sen in zip(res, tokenized_txt[i:j]):       
       ...
       final_res[idx]=np.transpose(np.matmul(R, np.transpose(avg)))
       idx=idx+1
